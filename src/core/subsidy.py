...
class SubsidyModule:
    ...
    def compute_subsidies(self, agents, round_contributions: dict) -> dict:
        ...
        si_agent_ids = {
            a.agent_id for a in agents if getattr(a, "institution_choice", "SFI") == "SI"
        }
        si_contributions = {
            aid: contrib for aid, contrib in round_contributions.items() if aid in si_agent_ids
        }
        ...
        total_punishments_this_round = sum(
            ...
            for a in agents
            if hasattr(a, "assigned_punishments") and a.agent_id in si_agent_ids
        )
        pool = int(
            total_punishments_this_round * parameters.PUNISHMENT_COST * parameters.SUBSIDY_FRACTION
        )
