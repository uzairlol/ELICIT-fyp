...
def main():
    ...
    if scenario_key == "ldf":
        ...
        if required_agents > 0 and required_agents != parameters.NUM_AGENTS:
            logger.info(
    ...
    def build_group_profile_list(total_agents):
        ...
        if configured_total != total_agents:
            ...
            if configured_total > 0:
                dev_n = round(total_agents * (dev_n / configured_total))
        ...
        if scenario_name == "ldf":
            ...
            developing_endowments = list(
                getattr(parameters, "LDF_DEVELOPING_INITIAL_ENDOWMENTS", [])
            )
            if developing_endowments:
                if len(developing_endowments) < developing_n:
                    developing_endowments.extend(
                        [float(developing_endowments[-1])]
                        * (developing_n - len(developing_endowments))
                    )
                else:
                ...
                developing_wealth = float(
                    getattr(
                        parameters,
                        "LDF_DEVELOPING_INITIAL_WEALTH",
                        parameters.DEVELOPING_INITIAL_WEALTH,
                    )
                )
            ...
            for _ in range(dev_n):
                profiles.append(
                    {
                        "group": "developed",
                        "initial_wealth": float(parameters.DEVELOPED_INITIAL_WEALTH),
                    }
                )
            for _ in range(developing_n):
                profiles.append(
                    {
                        "group": "developing",
                        "initial_wealth": float(parameters.DEVELOPING_INITIAL_WEALTH),
                    }
                )
    ...
    def apply_group_profile(agent, profile):
        ...
        initial_wealth = float(
            profile.get(
                "initial_wealth",
                parameters.DEVELOPED_INITIAL_WEALTH
                if group_name == "developed"
                else parameters.DEVELOPING_INITIAL_WEALTH,
            )
        )
    ...
    logger.info(
        f"Total LLM Cost (Local {parameters.LLM_MODEL}): ${api_client.get_total_cost():.6f}"
    )
