...
def _contribution_budget(agent):
    if _uses_climate_budget():
        return max(
            parameters.MIN_CONTRIBUTION, int(getattr(agent, "wealth", parameters.ENDOWMENT_STAGE_1))
        )
...
def _should_broadcast_country_types():
    ...
    return not batch_name.startswith("control")
...
def _received_tokens_from_effect(effect, effect_per_token):
    ...
    if effect_per_token <= 0:
        return 0
    return round(float(effect or 0) / effect_per_token)
...
def _build_stage0_card(agent, round_number, sc):
    ...
    if _uses_climate_budget():
        ...
        si_payoff_formula = "`stage1_payoff - amount_you_spend + rewards_you_receive - punishments_you_receive` (costs are deducted directly from your wealth)"
        key_tradeoff_desc = (
...
def _build_stage1_card(agent, group_state, sc):
    ...
    if getattr(agent, "anonymous_data_history", None):
        ...
        same_inst_contribs = [
            entry["contribution"]
            for entry in latest.get("anonymous_data", [])
            if entry.get("institution_choice") == getattr(agent, "institution_choice", None)
        ]
        if same_inst_contribs:
            prev_avg_str = (
                f"{sum(same_inst_contribs) / len(same_inst_contribs):.2f} {sc['currency_name']}"
            )
...
def _build_stage2_card(agent, group_state, sc, ordered_others=None):
    ...
    for member in others:
        label_id = (
            member.agent_id
            if not use_anonymity
            else agent.pseudonym_mapping.get(member.agent_id, -1)
        )
        ...
        group_type = (
            f", group={getattr(member, 'agent_group', 'unknown')}" if show_country_types else ""
        )

        deviation = contribution - avg_contrib
        free_rider_tag = (
            " [FREE-RIDER — contributed BELOW average]" if contribution < avg_contrib else ""
        )
        stated_reason = (
            getattr(member, "contribution_reasoning", "") or "No reasoning provided."
        ).strip()
    ...
    max_punishment = (
        agent.get_max_punishment_tokens()
        if hasattr(agent, "get_max_punishment_tokens")
        else parameters.MAX_PUNISHMENT_TOKENS
    )
    currency = sc["currency_name"]
...
def __build_prompt_prefix(agent, stage_text, round_number, sc):
    ...
    funding_info = (
        " (funded directly from wealth, no free endowment)"
        if _uses_climate_budget()
        else " endowment"
    )
...
def _append_belief_state(prompt, agent, sc, allowed_peer_ids=None):
    ...
    if agent.anonymous_data_history:
        ...
        for i, entry in enumerate(anonymous_data_list):
            ...
            country_str = (
                f"Country type: {entry.get('agent_group', 'unknown')}, "
                if show_country_types
                else ""
            )
            received_p_tokens = _received_tokens_from_effect(
                entry.get("received_punishments", 0), parameters.PUNISHMENT_EFFECT
...
def construct_institution_choice_prompt(agent, round_number):
    sc = get_scenario_config(parameters.SCENARIO)
    stage_text = "Stage 0 — Institution selection."
    ...
    if (
        parameters.CURIOSITY_ENABLED
        and parameters.CURIOSITY_BONUS_PROMPT
        and len(agent.history_institutions) >= 3
    ):
        last_3 = set(agent.history_institutions[-3:])
        if len(last_3) == 1:
            regime = next(iter(last_3))
...
def construct_contribution_prompt(agent, group_state):
    ...
    prompt = _agent_task_header(
        agent,
        "choose contribution amount",
        agent.round_number,
        f"Institution: {agent.institution_choice}.",
    )
    ...
    prompt += _json_response_block(
        "Contribution Choice", ldf_mode=_uses_climate_budget(), budget=budget
    )

    if (
        parameters.CURIOSITY_ENABLED
        and parameters.CURIOSITY_BONUS_PROMPT
        and len(agent.history_contributions) >= 3
    ):
...
def construct_punishment_prompt(agent, group_state):
    ...
    prompt = _agent_task_header(
        agent,
        "assign punishments and optional rewards",
        agent.round_number,
        f"Institution: {agent.institution_choice}.",
    )
    prompt += __build_prompt_prefix(agent, stage_text, agent.round_number, sc)
    ...
    card_text, target_labels = _build_stage2_card(
        agent, group_state, sc, ordered_others=ordered_others
    )
    prompt += card_text
    ...
    if _uses_climate_budget():
        amount_contract = f"- Amounts must be integers in {sc['currency_name']} on the same scale as contributions and wealth (no arbitrary caps)."
        budget_contract = (
        ...
        justify_contract = '- "justifications" MUST include a short sentence explaining your action for each target you chose to punish or reward.'
        reasoning_contract = (
        ...
        budget_contract = (
            f"- **BUDGET LIMIT:** Your total spend (punishment tokens x {parameters.PUNISHMENT_COST} + reward tokens x {parameters.REWARD_COST}) "
