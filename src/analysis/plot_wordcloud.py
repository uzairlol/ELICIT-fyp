...
def generate_wordcloud_grid(
    data, round_num, total_rounds, agent_ids, num_agents, category_name, text_extractor, output_dir
):
    # Determine grid size
    if num_agents == 7:
        cols, rows = 4, 2
    ...
    plt.suptitle(
        f"Word Clouds ({display_category}) - Round {round_num}",
        fontsize=14,
        fontweight="bold",
        y=0.98,
    )
...
def main():
    ...
    for round_idx, round_data in enumerate(data):
        ...
        agent_ids = sorted(agents.keys(), key=lambda x: int(x) if x.isdigit() else x)
