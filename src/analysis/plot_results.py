...
def plot_institutional_population(data, output_dir, filename_prefix):
    ...
    plt.plot(
        ...
        linestyle="-",
        color="g",
        label="Sanctioning Institution (SI)",
    )
    plt.plot(
        rounds,
        sfi_counts,
        ...
        linestyle="-",
        color="r",
        label="Sanction-Free Institution (SFI)",
    )
...
def plot_cumulative_payoff(data, output_dir, filename_prefix, scapegoat_id):
    seen_ids = set()
    for round_data in data:
        for str_aid in round_data.get("agents", {}):
            seen_ids.add(int(str_aid))

    agent_ids = sorted(seen_ids)
    rounds = [r.get("round_number", 0) for r in data]

    payoffs: dict[int, list[float]] = {aid: [] for aid in agent_ids}
...
def plot_punishments_received(data, output_dir, filename_prefix, scapegoat_id):
    ...
    punishments: dict[int, list[float]] = {aid: [] for aid in agent_ids}
...
def plot_agent_contributions(data, output_dir, filename_prefix, scapegoat_id):
    ...
    contributions: dict[int, list[float]] = {aid: [] for aid in agent_ids}
...
def plot_agent_reputations(data, output_dir, filename_prefix, scapegoat_id):
    ...
    reputations: dict[int, list[float]] = {aid: [] for aid in agent_ids}
...
def plot_assigned_punishments(data, output_dir, filename_prefix, scapegoat_id):
    ...
    assigned_punishments: dict[int, list[float]] = {aid: [] for aid in agent_ids}
...
def plot_llm_safety_metrics(data, output_dir, filename_prefix):
    ...
    plt.plot(
        ...
        linestyle="-",
        color="r",
        label="Parsing Failures (Fallbacks)",
    )
    plt.plot(
        rounds,
        rule_of_law_blocks,
        ...
        linestyle="-",
        color="orange",
        label="Rule of Law Blocks (Hallucinations)",
    )
...
def is_ldf_scenario_file(filename, data=None):
    ...
    if data:
        for r in data:
            if (
                r.get("ldf_pool_start", 0.0) > 0.0
                or r.get("ldf_contributions_total", 0.0) > 0.0
                or r.get("ldf_payouts_total", 0.0) > 0.0
            ):
                return True
...
def plot_ldf_pool_and_flows(data, output_dir, filename_prefix):
    ...
    ax1.bar(
        [i - width / 2 for i in x], contrib, width=width, color="#4C78A8", label="LDF Contributions"
    )
...
def plot_ldf_damage_and_coverage(data, output_dir, filename_prefix):
    ...
    coverage = []
    for g, p in zip(gross, payouts, strict=False):
    ...
    ax2.plot(
        ...
        marker="o",
        linewidth=2.0,
        label="Coverage Ratio (Payout/Gross)",
    )
...
def _group_round_totals(round_data):
    out: defaultdict[str, dict[str, float]] = defaultdict(lambda: {"contrib": 0.0, "payout": 0.0})
...
def plot_ldf_group_burden_split(data, output_dir, filename_prefix):
    ...
    run_dev = 0.0
    run_deving = 0.0
    for dc, dpc, dp, dpp in zip(
        dev_contrib, deving_contrib, dev_payout, deving_payout, strict=False
    ):
        run_dev += dp - dc
        run_deving += dpp - dpc
    ...
    ax1.plot(
        rounds, dev_payout, marker="x", linestyle="--", color="#B279A2", label="Developed Payouts"
    )
    ax1.plot(
        rounds,
        deving_payout,
    ...
    ax2.plot(
        rounds, cum_dev_net, marker="o", color="#E45756", label="Developed Cumulative Net Transfer"
    )
    ax2.plot(
        rounds,
        cum_deving_net,
        marker="o",
        color="#54A24B",
        label="Developing Cumulative Net Transfer",
    )
...
def _matching_ldf_condition_files(results_dir, selected_file):
    ...
    files = [
        f for f in os.listdir(results_dir) if f.endswith(".json") and scenario_token in f.lower()
    ]
...
def main():
    ...
    if not results_dir.exists():
        print(
            f"Error: Results directory '{results_dir}' not found. Have you run the simulation yet?"
        )
        return
    ...
    print(" 3. Cumulative Payoffs Tracking (_3_cumulative_payoffs.png)")
    print(" 4. Punishments Received - Being Targeted (_4_punishments_received.png)")
