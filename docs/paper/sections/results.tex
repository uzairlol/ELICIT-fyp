...
Figure~\ref{fig:rep_deltas} and Table~\ref{tab:event_study} present the primary event study with nonparametric bootstrap confidence intervals ($B{=}5000$).
...
All six cellwise 95\% intervals include zero, so uncertainty is large relative to the means.
Standardized effects versus zero are small ($|d|\le 0.17$).
...
Table~\ref{tab:did} compares affected agents with same-round controls.
The reported DiD is the mean across rounds of (affected mean $\Delta$ $-$ control mean $\Delta$).
Estimates are $-0.288$ for bad reputation, $-0.208$ for gossip, and $-0.143$ for reputation drops.
Bootstrap CIs over rounds are wide; only the reputation-drop interval excludes zero at 95\%.
Cohen's $d$ for affected versus control agent-round deltas ranges from $-0.10$ to $-0.18$.
A within-round label-shuffle permutation test ($R{=}5000$) yields two-sided $p{=}0.015$ (bad reputation), $p{=}0.039$ (gossip), and $p{=}0.072$ (reputation drop).
...
First events coincide with larger declines than repeats.
...
The qualitative sample comprises contribution rationales in rounds immediately after a bad-reputation or gossip event ($N{=}114$ agent-round texts).
Inclusion requires a lagged event flag and a non-empty contribution rationale.
Texts are tagged with a multi-label codebook.
\emph{Opportunistic} covers free-riding, self-interest, payoff maximization, or marginal-return language as the stated decision driver.
\emph{Conformity} covers peer or group-average behavior as the reason for the agent's own choice.
\emph{Reputation management} covers explicit standing, credibility, trust scores, or avoiding a free-rider label.
\emph{Named agents} requires reference to a specific peer identifier.
\emph{Future-rounds} covers intertemporal conservation without a stronger primary motif.
\emph{Gossip reference} requires explicit gossip or bulletin language.
A text may receive multiple tags.
Primary analysis reports tag counts (Table~\ref{tab:motifs}).

Coder~A applies regular-expression operationalizations of the codebook.
Coder~B independently applies the written definitions to a stratified 41\% subsample ($n{=}47$).
Percent agreement is $0.83$.
Cohen's $\kappa$ is $0.72$ for the mutually exclusive primary label and $0.91$ for opportunistic versus other.
Agreement is substantial for the focal opportunistic contrast.

\subsubsection{Motif frequencies}

Table~\ref{tab:motifs} summarizes multi-label motif counts.
...
\subsubsection{Contribution lexicon}

Figure~\ref{fig:wordclouds} shows contribution-rationale wordclouds by institution.
These clouds summarise ambient Stage-1 language across the run, so they situate the post-event motif counts in a broader lexical field.
SI contribution talk emphasises strategy, self-interest, payoff, and maximisation.
SFI contribution talk emphasises incentives, immediacy, and institutional framing.
Reputation-repair and fairness vocabulary are not visually dominant in either cloud.
The lexical picture aligns with the coded motif imbalance.
Agents narrate contribution choices in an opportunistic register even outside the event subsample.
That register helps explain why social stigma is followed by withdrawal language more often than by image-repair language.
Institution labels remain collinear with country group, so the SI/SFI lexical split is descriptive of the joint assignment rather than a causal institution effect.
Appendix Figure~\ref{fig:app_keyness} reports the corresponding keyness contrast.
...
\caption{SI contribution rationales.}
\label{fig:wc_si}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\linewidth}
...
\caption{SFI contribution rationales.}
\label{fig:wc_sfi}
\end{subfigure}
\caption{Contribution-stage wordclouds by institution.
Payoff and strategy terms dominate SI talk; incentive and immediacy terms dominate SFI talk.
Repair-oriented vocabulary is comparatively weak in both clouds.}
