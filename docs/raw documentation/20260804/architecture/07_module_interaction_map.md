# 07 â€” Module Interaction Map (20260804)

How modules chain within and across rounds. Mermaid diagrams + textual flows.
No behavioural verdicts about the 20260804 run.
...
`observation â†’ reasoning â†’ contribution â†’ reputation â†’ gossip â†’ proposal â†’ vote â†’ institutional change â†’ later contribution`
...
| Reputation | ToM after round | Peer scores â†’ mean \(\rho_i\) |
| Gossip | GossipModule | Low scores published to othersâ€™ next-round prompts |
| Proposal | Democracy (interval) | Agents propose parameter changes |
| Vote | Democracy | Plurality selects winning proposal |
| Institutional / rule change | `setattr(parameters, â€¦)` | Live rule update (not SIâ†”SFI switch in LDF â€” membership stays group-forced) |
| Later contribution | Next rounds | New prompts under updated rules + social signals |

**LDF-mode caveat:** â€œInstitutional changeâ€ in democracy means **parameter** change (sanction strength, subsidy, LDF payout weights, etc.), **not** free switching between SI and SFI. Membership remains developedâ†’SI / developingâ†’SFI.
...
- Subsidy recycles a fraction of SI punishment **token costs** to top contributors â€” an implemented transfer, not LLM-chosen.
...
| Not claimed in this document | Norm emergence; â€œstable cooperationâ€; real-world LDF effectiveness |
