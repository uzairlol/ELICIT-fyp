...
and climatic-shock event study for the locked 20260804 run.

Run from repo root:
  python "docs/raw documentation/20260804/scripts/analyze_20260804_contributions.py"
"""
...
def make_plots(df: pd.DataFrame, round_sum: pd.DataFrame, deltas: pd.DataFrame) -> None:
    ...
    for inst, color in [("SI", "#5B4B8A"), ("SFI", "#C47B2C")]:
        ...
        # approximate band using meanÂ± not IQR center; plot IQR half around median
        ax.fill_between(
            sub["round_number"],
            ...
            color=color,
            alpha=0.15,
            label=f"{inst} Â±IQR/2",
        )
    for s in SHOCK_ROUNDS:
        ax.axvline(s, color="#AA2222", ls="--", lw=1)
    ax.set_xlabel("Round")
    ax.set_ylabel("Median prop_of_wealth")
    ax.set_title("Median proportional contribution (Â± IQR/2 band)")
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(PLOTS / "contrib_median_prop_iqr.png", dpi=150)
    plt.close(fig)

    # 3. Absolute mean contribution (log scale) â€” wealth confound visible
    fig, ax = plt.subplots(figsize=(10, 5))
    for inst, color in [("SI", "#5B4B8A"), ("SFI", "#C47B2C")]:
        sub = round_sum[round_sum["institution_choice"] == inst]
    ...
    for inst in ("SI", "SFI"):
        ...
        ax.set_title(f"Individual prop trajectories â€” {inst}")
        ax.legend()
        fig.tight_layout()
    ...
    for ax, shock in zip(axes, SHOCK_ROUNDS):
        ...
        ax.set_title(f"Shock R{shock}: postâˆ’pre prop")
        ax.set_ylabel("Î” prop_of_wealth")
