...
and per-agent strategy profiles for 20260804.

Run from repo root:
  python "docs/raw documentation/20260804/scripts/analyze_20260804_reputation_gossip.py"
"""
...
def strategy_profiles(panel: pd.DataFrame, events: pd.DataFrame, motifs: pd.DataFrame) -> pd.DataFrame:
    ...
    for aid, g in panel.groupby("agent_id"):
        ...
        try:
            ...
            shock_note = "; ".join(
                f"R{int(r.shock_round)} Î”={r.delta_post_minus_pre:.3f}" for _, r in sh.iterrows()
            )
        except Exception:
            shock_note = ""
