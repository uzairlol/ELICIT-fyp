...
def ev(table: str, record: str = "n/a") -> str:
    return f"[Evidence: `tables/{table}` | run={RUN} | round=n/a | agent=n/a | record={record}]"
...
**Opening claim.** Mean prop is nearly identical across institutions; absolute contributions differ by ~18x because wealth stocks differ. No causal institution contrast is identified.
...
""",
    ...
    # Qualitative
    imm = [e for e in p4.get("event_summary_head", []) if e.get("horizon") == "imm"]
    imm_tbl = "\n".join(
...
""",
    ...
    write(
        ...
        """# Paper Story - Seed2 Extension & Cross-Seed Bridge
...
1. Add a short **multi-seed robustness** subsection with the event-study table by seedxinstitution.
