...
for the locked 20260731 run.

Run from repo root:
  python "docs/raw documentation/20260731/scripts/analyze_20260731_dashboard_dimensions.py"
"""
...
def shock_absorber_2x2() -> pd.DataFrame:
    ...
    for aid, g in c.groupby("agent_id"):
        ...
        def delta(shock, _g=g):
            pre = _g[_g["round_number"] == shock - 1]["prop_of_wealth"]
            post = _g[_g["round_number"] == shock + 1]["prop_of_wealth"]
...
def make_plots(gap_df: pd.DataFrame, sanc_df: pd.DataFrame, fund_round: pd.DataFrame) -> None:
    ...
    ax.set_title("Developed-developing wealth gap")
    fig.tight_layout()
