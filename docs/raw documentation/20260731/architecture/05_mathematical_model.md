# 05 — Mathematical Model (20260731)
...
Stochastic branch (if enabled): Bernoulli(`CLIMATE_SHOCK_BASE_PROB`) then Uniform severity — **not** the path for deterministic schedule runs.
...
with \(\bar{w}\) = mean developing wealth (floored wealth ≥ 1 in ratio).
...
- Outcome: plurality; ties → `random.choice`
