# 04 — System Architecture (20260731)
...
[Evidence: `docs/raw documentation/20260731/00_project_memory.md` | run=20260731_013853 | round=n/a | agent=n/a | record=Confirmed_analysis_run]
...
2. **Institution assignment**
   - Climate/LDF mode (**this run**): **forced** — developed → SI, developing → SFI  
...
3. **Stage 1 — Contributions** (`collect_contributions`) — LLM amounts (**prompted**), clamped to stage-1 cap (**implemented**)
4. **Public-goods distribution** (`distribute_public_goods`) — share formula (**implemented**)
5. **Stage 2 — SI only** punish/reward assign + apply (**prompted** allocations; **implemented** effects)
6. **Subsidy** (if enabled) — top SI contributors (**implemented**)
...
10. **ToM audit + reputation + gossip** (if enabled) — scores **prompted**; mean reputation / bulletin filter **implemented**
11. **Democracy** every `DEMOCRACY_INTERVAL` rounds — proposals/votes **prompted**; tally/apply **implemented**
...
| Step | Detail |
|------|--------|
| Input | Target’s stated contribution reasoning vs actual contribution |
| Decision | Pairwise LLM trust score 1–10 (**prompted**; score-only contract in current ToM) |
...
| Selection | Scores ≤ `GOSSIP_TRIGGER_SCORE`; top `MAX_GOSSIP_ITEMS` worst |
...
| Results gap | Bulletin **not** written into the 20260731 JSON export |
...
| Win rule | Plurality (`max` votes); ties broken randomly — not strict majority |
| Apply | `setattr(parameters, rule, new_value)` with clamp to ~\[0.1×, 10×\] current |
...
Round payoff (climate/LDF): Stage-1 net + Stage-2 net + subsidy + LDF payout − climate damage.  
...
| `history_institutions` / `history_contributions` | ≤10; last 3 shown on cards |
...
Filename flags (`scnldf`, `sh1`, `ldf1`) imply LDF scenario + shocks + LDF enabled via experiment CLI. Current `parameters.py` defaults still show `LDF_ENABLED=False` / `CLIMATE_SHOCK_ENABLED=False` — those defaults are **not** the historical run switches. Prefer filename + in-file shock/LDF fields as evidence of what ran.
