# 26 — LDF Context and Multi-Agent Motivation (20260731)

Links the real Fund for responding to Loss and Damage (FRLD) to why a multi-agent simulation is a suitable *method* — without claiming the 20260731 run predicts real-country outcomes.

External claims cite verified sources listed in `28_external_sources.md`. Simulation claims cite repository evidence.

---

## A. Real-world LDF / FRLD — verified timeline (do not conflate stages)
...
| **Purpose (Governing Instrument)** | Assist particularly vulnerable developing countries in responding to **economic and non-economic** loss and damage, including extreme and slow-onset events; channel for new/additional multilateral finance | Governing Instrument §§1–3 in 1/CP.28 annex |
| **Pledges ≠ deposits ≠ disbursement** | COP27 left many contribution/access details open; operationalisation created governance machinery — **not** automatic full capitalisation or universal access | UNEP COP27 summary; UNFCCC Fund page |

**Terminology used carefully:** “establishment” (COP27) ≠ “operationalisation / Governing Instrument / Board / interim host” (COP28+) ≠ “pledge” ≠ “paid-in contribution” ≠ “disbursement to recipients”.

[External: `theory/28_external_sources.md` | ids E1–E5]
...
- Board autonomy vs World Bank FIF hosting conditions (1/CP.28 ¶¶17–24).
- Complementarity with other climate funds and processes.

These are **institutional design problems under strategic interaction and incomplete information** — the class of problem multi-agent models can explore mechanistically.
...
[Evidence: `00_project_memory.md` | run=20260731_013853 | round=5,10 | agent=n/a | record=shocks]  
...
1. **Repeated strategic interaction** — 30 rounds of contribution and (for SI) enforcement.
2. **Heterogeneous incentives** — developed/SI vs developing/SFI wealth, vulnerability, Stage-2 rights.
3. **Reputation and expectations** — ToM, reputation, gossip (even if repair fails empirically here).
4. **Evolving governance rules** — constitutional parameter votes.
5. **Incomplete information** — hidden pool; no same-round peer contributions.
6. **Collection ≠ effective redistribution** — pool may be insufficient; coverage capped; agents cannot verify stock.
7. **Second collective-action problem** — costly SI enforcement + costless democracy.
8. **Formal rules × informal norms** — parameters vs reasoning language (Prompts 5–7: rules drift; norms weak).
9. **Shocks alter cooperation** — R5/R10 disturb prop paths without permanent mean collapse.

[Evidence: `synthesis/22_cooperation_stability_assessment.md` | run=20260731_013853 | round=n/a | agent=n/a | record=shock_recoveries]  
[Evidence: `qualitative_analysis/17_enforcement_as_public_good.md` | run=20260731_013853 | round=n/a | agent=n/a | record=second_order]
...
- Dual-use Stage-1→LDF deposit collapses two real instruments into one action.

**Therefore:** the run can speak to **mechanisms** (contribution under opacity, asymmetric enforcement, rule drift) — not to whether the real FRLD “works.”
...
- Observe **path dependence** in parameters and contribution paths (group-mean autocorr≈0.03 here).
- Test **interactions of formal and informal institutions** (democracy + Stage-2 + reputation).
- Examine **counterfactual governance structures** via code flags (SI/SFI, LDF on/off, shocks) — even when this pack analyses one Full condition.
